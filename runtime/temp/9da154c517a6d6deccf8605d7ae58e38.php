			...
			<div class="main-content">
                ...
                <div class="block-area" id="horizontal">
                    <div class="block-title">
                        <p class="pull-right">
                            <a href="<?php echo url('index'); ?>"><span class="label label-primary">返回列表</span></a>
                        </p>
                        <div class="clearfix"></div>
                    </div>
                    <!-- <h3 class="block-title"><?php echo $title; ?></h3> -->
                    <form class="form-horizontal form-validation-9"" role="form" method="post" action="<?php echo url($actionName); ?>">
                        <div class="form-group">
                            <label for="role_name" class="col-md-2 control-label">角色名称</label>
                            <div class="col-md-10">
                                <input type="text" class="form-control input-sm" id="role_name" value="<?php echo $editData['role_name']; ?>" disabled="disabled">
                            </div>
                        </div>
                        <div class="form-group">
                            <label for="role_desc" class="col-md-2 control-label">角色权限</label>
                            <div class="col-md-10" style="margin-top: 7px;">
                                <?php if(is_array($tree) || $tree instanceof \think\Collection || $tree instanceof \think\Paginator): $i = 0; $__LIST__ = $tree;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$value): $mod = ($i % 2 );++$i;?>
                                    <div class="permission-parent">
                                        <label class="">
                                            <input type="checkbox" class="permission-parent-input" id="permission-<?php echo $value['id']; ?>"> <?php echo $value['permission_name']; ?>
                                        </label>
                                    </div>
                                    <hr class="whiter m-t-20" style="margin-top: 5px;margin-bottom: 15px;" />
                                    <?php if(!(empty($value['children']) || (($value['children'] instanceof \think\Collection || $value['children'] instanceof \think\Paginator ) && $value['children']->isEmpty()))): ?>
                                    <div class="row" style="padding: 0 25px 0;">
                                        <?php if(is_array($value['children']) || $value['children'] instanceof \think\Collection || $value['children'] instanceof \think\Paginator): $i = 0; $__LIST__ = $value['children'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?>
                                            <label class="col-md-2" style="font-weight: 10;font-size: 13px;">
                                                <input type="checkbox" name="data[<?php echo $value['id']; ?>][]" value="<?php echo $val['id']; ?>" class="permission-child permission-parent-<?php echo $value['id']; ?>" id="permission-child-<?php echo $val['id']; ?>" data-parent-id="<?php echo $value['id']; ?>" <?php echo (isset($editData['role_permission'][$value['id']]) && !empty($editData['role_permission'][$value['id']])) && (in_array($val['id'],$editData['role_permission'][$value['id']])) ? ' checked="checked" ' : ''; ?>> <?php echo $val['permission_name']; ?>
                                            </label>
                                        <?php endforeach; endif; else: echo "" ;endif; ?>
                                    </div>
                                    <?php endif; ?>
                                    <hr class="whiter m-t-20" style="margin-top: 10px;" />
                                <?php endforeach; endif; else: echo "" ;endif; ?>
                            </div>
                        </div>
                        <div class="form-group">
